...
public class Requests {
	...
	// ======================================
	public static ResultSet AUTOCOMPLETE_PATIENT_NAME(Connection conn, String patient_name) throws SQLException {
		PreparedStatement query = (PreparedStatement) conn.prepareStatement("SELECT nom,prenom FROM Patients WHERE nom LIKE (?) OR 	prenom LIKE (?)");
		query.setString(1, patient_name);
		query.setString(2, patient_name);
	...
	public static List<Object> SEARCH_DMP_BY_FIRSTNAME(Connection conn, String firstName) throws SQLException {
		...
		while (set.next()) {
			...
			ResultSet antec = SELECT_Antecedent(conn, num_dossier);
			ResultSet doc = SELECT_Doc(conn, num_dossier);
			//les 5 premiers elements correspondent au 1er numero de dossier, les 5 autres aux 2ieme, etc...
			result.add(presc);
			result.add(elem);
			result.add(epi);
			result.add(antec);
